...
function isWord (str) {
    if (dictionary.some((element) => str == element)) {
        return true
    }
}

function undoSwaps (str) {
    
    for( let i = 0; i < str.length-1; i++) {
        var testArr = str.split("");
        testArr[i] = str.charAt(i+1);
        testArr[i+1] = str.charAt(i);
        var testString = testArr.join("");
        if (isWord(testString)) {
            return testString
        }
    }
}

function deleteRepeats (str) {
    for( let i = 0; i < str.length-1; i++) {
        var testArr = str.split("");
        if (testArr[i] == testArr[i+1]) {
            testArr.splice(i,1);
            if(isWord(testArr.join(""))) {
                i = str.length-1;
            }
        }
    }
...
function qwertyProblems (str) {
    for(let i = 0; i < str.length-1; i++) { //loop through the letters of the word
        var testArr = str.split("");
...
function autoCorrect (str) {
    if(isWord(str)) {
        return `You spelled "${str}" correctly. Well done!`;
    };
    ...
    const swaps = undoSwaps(str);
    if (swaps) {
        return `I think you'll find that's spelled "${swaps}"`
    }

    const qwerty = qwertyProblems(str);
    if (qwerty) {
        return `I think you'll find that's spelled "${qwerty}"`
    }

    const repeats = deleteRepeats(str);
    if (repeats) {
        return `I think you'll find that's spelled "${repeats}"`
    }
return "Either that spelling is so bad, even I can't fix it, or that's not one of the thousand most common words"
}
...
function run(input) {
    const output = autoCorrect(input);
    $("#output").html(output);
